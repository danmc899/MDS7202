...
class APIClient:
    ...
    def get_available_ids(self) -> Dict:
        """
        Obtiene los IDs disponibles de clientes y productos
        
        Returns:
            Diccionario con listas de customer_ids y product_ids
        """
        try:
            return self._make_request("GET", "/api/v1/data/available-ids")
        except Exception as e:
            logger.warning(f"No se pudieron obtener IDs disponibles: {e}")
            # Retornar listas vacías como fallback
            return {"customer_ids": [], "product_ids": []}
    
    def get_customer_products(self, customer_id: int) -> Dict:
        """
        Obtiene los productos disponibles para un cliente específico
        
        Args:
            customer_id: ID del cliente
            
        Returns:
            Diccionario con product_ids disponibles para ese cliente
        """
        try:
            return self._make_request("GET", f"/api/v1/data/customer-products/{customer_id}")
        except Exception as e:
            logger.warning(f"No se pudieron obtener productos del cliente {customer_id}: {e}")
            return {"customer_id": customer_id, "product_ids": [], "total": 0}
