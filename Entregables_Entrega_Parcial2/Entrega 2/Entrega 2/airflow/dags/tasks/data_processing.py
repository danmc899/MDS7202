...
Pipeline de procesamiento de datos para MLOps
"""
import pandas as pd
import numpy as np
...
def create_target_variable(df_transacciones, sample_negatives=True, negative_ratio=10.0):
    """
    Crea la variable target balanceada
    
    Args:
        df_transacciones: DataFrame con transacciones
        sample_negatives: Si True, genera ejemplos negativos
        negative_ratio: Ratio de negativos por cada positivo (default 10:1)
    ...
    if sample_negatives:
        ...
        logger.info(f"🎲 Generando {num_negatives:,} ejemplos negativos (ratio {negative_ratio}:1)...")
        ...
        n_positivos = len(df_positives)
        if len(df_negatives) > n_positivos:
            logger.info(f"⚖️ Aplicando undersampling con RandomUnderSampler...")
            logger.info(f"   Antes: {len(df_negatives):,} negativos")
            
            # Combinar primero
            df_combined = pd.concat([df_positives, df_negatives], ignore_index=True)
            
            # Preparar para imblearn (necesita X, y)
            X_temp = df_combined[['customer_id', 'product_id', 'purchase_week']].values
            y_temp = df_combined['compra_semanal'].values
            ...
            df_result = pd.DataFrame(X_resampled, columns=['customer_id', 'product_id', 'purchase_week'])
            df_result['compra_semanal'] = y_resampled
            
            logger.info(f"   Después: {(df_result['compra_semanal']==0).sum():,} negativos")
        else:
            df_result = pd.concat([df_positives, df_negatives], ignore_index=True)
        
        logger.info(f"📊 Dataset balanceado:")
        logger.info(f"   - Positivos: {(df_result['compra_semanal']==1).sum():,}")
        logger.info(f"   - Negativos: {(df_result['compra_semanal']==0).sum():,}")
        logger.info(f"   - Total: {len(df_result):,}")
...
def process_data(**context):
    """
    Tarea principal de procesamiento de datos
    """
    ...
    # Crear variable target (ratio 10:1, luego undersample a 1:1)
    df_target = create_target_variable(df_transacciones, sample_negatives=True, negative_ratio=10.0)
