...
def generate_predictions(**context):
    ...
    week_id = ti.xcom_pull(key='week_id', task_ids='extract_data')
    
    logger.info(f"Week ID para esta ejecución: {week_id}")
    ...
    # Guardar predicciones - estructura organizada
    predictions_base = Path("/opt/airflow/data/predictions")
    stats_dir = predictions_base / "stats"
    submissions_dir = predictions_base / "submissions"
    ...
    # Guardar estadísticas en subdirectorio stats/
    # Incluir week_id en el nombre para identificar claramente qué datos se usaron
    stats = {
    ...
    stats_filename = f"prediction_stats_{week_id}_{timestamp}.json"
    with open(stats_dir / stats_filename, 'w') as f:
        json.dump(stats, f, indent=2)
    logger.info(f"   ✓ Estadísticas guardadas: {stats_filename}")
    ...
    # Crear DataFrame de submission con IDs como enteros
    df_submission = df_predictions_positive[['customer_id', 'product_id']].copy()
    df_submission['customer_id'] = df_submission['customer_id'].astype(int)
    df_submission['product_id'] = df_submission['product_id'].astype(int)
    
    # Ya está ordenado por probabilidad descendente (del paso anterior)
    
    # Guardar CSV sin header en subdirectorio submissions/
    # Formato: submission_{week_id}_{timestamp}.csv
    submission_csv_path = submissions_dir / f"submission_{week_id}_{timestamp}.csv"
    df_submission.to_csv(submission_csv_path, index=False, header=False)
    logger.info(f"   ✓ CSV de submission guardado: {submission_csv_path}")
    logger.info(f"   ✓ Total filas: {len(df_submission):,}")
    
    # Crear ZIP con el CSV en submissions/
    submission_zip_path = submissions_dir / f"submission_{week_id}_{timestamp}.zip"
    with zipfile.ZipFile(submission_zip_path, 'w', zipfile.ZIP_DEFLATED) as zipf:
        # Agregar el CSV al ZIP con nombre simple
        zipf.write(submission_csv_path, "submission.csv")
    
    logger.info(f"   ✓ ZIP de submission creado: {submission_zip_path}")
    logger.info(f"")
    logger.info(f"🎯 ARCHIVO PARA CONDABENCH: {submission_zip_path}")
    logger.info(f"   Week ID: {week_id}")
    logger.info(f"   Contiene: submission.csv ({len(df_submission):,} predicciones positivas)")
    
    # Pushear información
    ti.xcom_push(key='num_positive_predictions', value=len(df_predictions_positive))
    ti.xcom_push(key='submission_zip_path', value=str(submission_zip_path))
    ti.xcom_push(key='week_id', value=week_id)
